export const services = [
	{
		id: '1',
		image:
			'https://res.cloudinary.com/playhouse-media-group/image/upload/v1641899583/portfolio/ui-ux_w59ezb.webp',
		name: 'Web Development',
		slug: 'web-development',
		description:
			'We build responsive websites for your business or organization that are user friendly and easy to use,such that they deliver leads, clients, and revenue. We use the latest technologies to make your website look great on all devices. We design and develop websites ',
		details: [
			{
				id: '1',
				title: 'Responsive Design',
				description:
					'We build responsive websites for your business or organization that are user friendly and easy to use,such that they deliver leads, clients, and revenue. We use the latest technologies to make your website look great on all devices. We design and develop websites ',
			},
			{
				id: '2',
				title: 'E-Commerce website',
				description:
					'If your product or services needs to be sold online, we will guide you through the process developing and setting up the shopping including the selection of the payment gateway provider. We will also provide assistance with setting up the catalogue.',
			},
			{
				id: '3',
				title: 'WordPress website',
				description:
					'We build responsive websites for your business or organization that are user friendly and easy to use,such that they deliver leads, clients, and revenue. We use the latest technologies to make your website look great on all devices. We design and develop websites ',
			},
			{
				id: '4',
				title: 'Landing Page',
				description:
					'We build responsive websites for your business or organization that are user friendly and easy to use,such that they deliver leads, clients, and revenue. We use the latest technologies to make your website look great on all devices. We design and develop websites ',
			},
			{
				id: '5',
				title: 'Web Hosting',
				description:
					'We will register your domain and set up the hosting for you. If you have an existing domain name hosted elsewhere, we will assist in moving your site. Alternatively, if you prefer utilizing your own domain, we can implement the website solution at your preferred location.',
			},
			{
				id: '6',
				title: 'Website Maintenance',
				description:
					'We provide website maintainance to keep your website at the forefront of online marketing. We use the latest technologies to make your website look great on all devices. We design and develop websites ',
			},
			{
				id: '7',
				title: 'Web Optimization',
				description:
					'We build responsive websites for your business or organization that are user friendly and easy to use,such that they deliver leads, clients, and revenue. We use the latest technologies to make your website look great on all devices. We design and develop websites ',
			},
		],
	},
	{
		id: '2',
		image:
			'https://res.cloudinary.com/playhouse-media-group/image/upload/v1641899581/portfolio/search-engine-optimization-01_t1b4f1.webp',
		name: 'Search Engine Optimization',
		slug: 'search-engine-optimization',
		description:
			'Search Engine Optimization will improve your online presence and increase visitors to your website. We will optimize your website for search engines such as Google, Bing, Yahoo, and others which will help you to get more traffic to your website. ',
		details: [
			{
				id: '1',
				title: 'SEO Assessment',
				description:
					'A critical analysis from the ground up from design infrastructure & content to link profile & brand strength ',
			},
			{
				id: '2',
				title: 'Business Profile Listings',
				description:
					'These are influential, industry based directories that every business should target to gain traction in Local SEO.',
			},
			{
				id: '3',
				title: 'Extensive Keyword Research',
				description:
					'The most important step in SEO, our keyword research is exhaustive and focuses on "conversion keywords" that bring leads.',
			},
			{
				id: '4',
				title: 'Infographics',
				description:
					'Give your content an engaging twist! Our designers will craft promotional infographics that get more conversions.',
			},
			{
				id: '5',
				title: 'Link-building Services',
				description:
					'We build links on authoritative sites that ultimately link back to your website, leveraging their credibility and improving SEO.',
			},
		],
	},
	{
		id: '3',
		image:
			'https://res.cloudinary.com/playhouse-media-group/image/upload/v1641899581/portfolio/digital_vx03l2.webp',
		name: 'Digital Marketing',
		slug: 'digital-marketing',
		description:
			'We offer Digital Marketing services to market your business and increase traffic and engagement. We will help you to increase your website traffic and increase your website visitors and convert them into leads.',
		details: [
			{
				id: '1',
				title: 'Google Ads Marketing',
				description:
					'We offer Digital Marketing services to market your business and increase traffic and engagement. We will help you to increase your website traffic and increase your website visitors and convert them into leads.',
			},
			{
				id: '2',
				title: 'Linkedin Ads Marketing',
				description:
					'We offer Digital Marketing services to market your business and increase traffic and engagement. We will help you to increase your website traffic and increase your website visitors and convert them into leads.',
			},
			{
				id: '3',
				title: 'Facebook Ads Marketing',
				description:
					'We offer Digital Marketing services to market your business and increase traffic and engagement. We will help you to increase your website traffic and increase your website visitors and convert them into leads.',
			},
			{
				id: '4',
				title: 'Instagram Ads Marketing',
				description:
					'We offer Digital Marketing services to market your business and increase traffic and engagement. We will help you to increase your website traffic and increase your website visitors and convert them into leads.',
			},
		],
	},
	{
		id: '4',
		image:
			'https://res.cloudinary.com/playhouse-media-group/image/upload/v1641899581/portfolio/digital_vx03l2.webp',
		name: 'Microsoft 365',
		slug: 'microsoft-365',
		description:
			'We offer Microsoft 365 services to provide your business with the best office experience. This comes with a wide range of services such as premium email, calendar, and more.',
		details: [
			{
				id: '1',
				title: 'Microsoft Office 365',
				description:
					'This service comes with different tools for your day to day running of your busuness such as Microsoft Word, Microsoft Excel, Microsoft PowerPoint and more.',
			},
			{
				id: '2',
				title: 'Microsoft Exchange',
				description:
					'This service provides you with the ability to send and receive emails and other communication tools such as calendar, contacts, it also provides you with 50GB of storage space per user.',
			},
			{
				id: '3',
				title: 'Microsoft OneDrive',
				description:
					'This service provides you with the ability to store and share files and folders with 1000GB of Cloud Storage space per user.',
			},
			{
				id: '4',
				title: 'Microsoft Teams',
				description:
					'This service provides you with the ability to communicate with your team members and customers in real time.',
			},
		],
	},
];
